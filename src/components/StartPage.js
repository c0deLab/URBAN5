...
export default class StartPage extends React.Component {
  ...
  getButton = id => {
    ...
    const title = dateStr;
    return (<li key={id}><button type="button" onClick={() => this.onButtonClick(id)}>{title}</button></li>);
  }
    ...
    return (
      <div style={{ width: '1024px', height: '100%', float: 'left', padding: '80px' }}>
        ...
        <ul style={{ lineHeight: '28px' }}>
          <li key="new"><button type="button" onClick={() => this.onButtonClick('new')}>NEW</button></li>
